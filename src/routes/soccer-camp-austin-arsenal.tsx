...
/* ---------------- Pathway to London ---------------- */
function PathwayToLondon() {
  const steps = [
    {
      icon: ClipboardList,
      step: "Step 1",
      title: "Register for camp",
      text: "Secure your spot in the Austin camp and start training the Arsenal Way.",
    },
    {
      icon: Trophy,
      step: "Step 2",
      title: "Get invited",
      text: "Top performers are invited to exclusive Arsenal Football Development clinics.",
    },
    {
      icon: Plane,
      step: "Step 3",
      title: "Fly to London",
      text: "Selected players travel to the UK for unique clinics hosted by the club.",
    },
  ];
  return (
    <section className="mx-auto max-w-7xl px-4 py-20 md:px-6 md:py-28 lg:px-8">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <p className="mt-4 text-base text-muted-foreground md:text-lg">
          Arsenal invites standout campers to once-in-a-lifetime UK soccer
          clinics — the ultimate step in a young player's development journey.
        </p>
      </div>
      ...
      <div className="mt-14 grid gap-6 md:grid-cols-3">
        {steps.map((s, i) => (
          <div
            key={s.step}
            className="relative overflow-hidden rounded-3xl border-2 border-accent bg-card p-7 shadow-sm"
          >
            <span className="absolute right-5 top-5 font-display text-5xl font-extrabold text-accent/60">
              0{i + 1}
            </span>
            <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-cta text-cta-foreground">
              <s.icon className="h-5 w-5" />
            </span>
            <div className="mt-5 text-[11px] font-semibold uppercase tracking-widest text-cta">
              {s.step}
            </div>
